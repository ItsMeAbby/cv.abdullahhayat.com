...
export const Link = ({ href, children }: { href: string; children: React.ReactNode }) => {
  return (
    <a href={href} className="group relative leading-5" target="_blank" rel="noopener noreferrer">
      {children}
      <span className="w-full h-px bg-sky-400 absolute bottom-0 left-0 group-hover:bg-sky-500 transition-colors" />
    </a>
  );
};
...
export const SectionList = ({ items }: { items: React.ReactNode[] }) => {
  return (
    <ul className="ml-4 print:ml-5 list-disc marker:text-sky-500 text-justify">
      {items.map((item, idx) => (
        <li key={idx}>{item}</li>
      ))}
    </ul>
